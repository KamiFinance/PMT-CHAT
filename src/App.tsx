...
export default function App() {
    ...
    if (Object.keys(savedMsgs).length > 0) {
      ...
      Object.entries(savedMsgs).forEach(([addr, list]) => {
        ...
        normalized[key] = [...(normalized[key] ?? []), ...deduped.map(m => {
          ...
          // Reconstruct gifUrl from gifId for old messages where URL was lost
          if (m.type === 'gif' && !m.gifUrl && (m as any).gifId) {
            m = { ...m, gifUrl: `https://media.giphy.com/media/${(m as any).gifId}/giphy.gif` };
          }
  ...
  const sendMsg = useCallback(async (input: string | Partial<Message>) => {
    ...
    const isGif   = typeof input === 'object' && input.type === 'gif';
    ...
    const preview = isVoice ? '🎙 Voice message' : isImage ? '🖼 Image' : isFile ? `📄 ${(input as Message).fileName ?? 'File'}` : isVideo ? '🎬 Video' : isGif ? ((input as any).isSticker ? '✨ Sticker' : '🎞 GIF') : textContent;
...
Answer questions about PMT, PMTchain, the app, or anything else the user asks.`, messages: [...history, { role: 'user', content: userMsg }] }),
    ...
    if (!activeRef.current.isGroup && !activeRef.current.isAI && !isDemo && walletRef.current?.address) {
      ...
      const msgContent = isVoice ? '🎙 Voice message' : isImage ? '🖼 Image' : isFile ? `📄 ${(input as Message).fileName ?? 'File'}` : isVideo ? '🎬 Video' : isGif ? ((input as any).isSticker ? '✨ Sticker' : '🎞 GIF') : textContent;
      ...
      try {
        ...
        })()), ...((isImage || isFile) && { ipfsCid: (input as Message).ipfsCid ?? null, b64Data: (input as Message).b64Data ?? null, mediaMsgId: (input as Message).mediaMsgId, imgMsgId: (input as Message).imgMsgId, fileName: (input as Message).fileName, fileSize: (input as Message).fileSize, mimeType: (input as Message).mimeType }), ...(isVideo && { ipfsCid: (input as Message).ipfsCid ?? null, ipfsUrl: (input as Message).ipfsUrl ?? null, fileName: (input as Message).fileName, fileSize: (input as Message).fileSize, mimeType: (input as Message).mimeType }), ...(isGif && { gifId: (input as any).gifId ?? null, gifUrl: (input as Message).gifUrl ?? null, gifWidth: (input as Message).gifWidth ?? null, gifHeight: (input as Message).gifHeight ?? null, isSticker: (input as Message).isSticker ?? false, title: (input as Message).title ?? '' }), from: w.address, fromName: profileRef.current?.name || w.username || w.address.slice(0, 8), fromAvatarUrl: (() => { const av = profileRef.current?.avatarUrl; return av?.startsWith('http') ? av : profileRef.current?._thumbUrl ?? null; })(), fromBio: profileRef.current?.bio ?? '', time: now(), block, hash: msg.hash, confirms: 0, ts: Date.now() };
    ...
    if (activeRef.current.isGroup && !isDemo && walletRef.current?.address) {
      ...
      const inboxMsg = {
        ...
        ...(isVideo && { ipfsCid: (input as Message).ipfsCid ?? null, ipfsUrl: (input as Message).ipfsUrl ?? null, fileName: (input as Message).fileName, fileSize: (input as Message).fileSize, mimeType: (input as Message).mimeType }), ...(isGif && { gifId: (input as any).gifId ?? null, gifUrl: (input as Message).gifUrl ?? null, gifWidth: (input as Message).gifWidth ?? null, gifHeight: (input as Message).gifHeight ?? null, isSticker: (input as Message).isSticker ?? false, title: (input as Message).title ?? '' }),
  ...
  useEffect(() => {
    ...
    // Don't prompt external wallet users (MetaMask/WalletConnect/Tangem)
    // privateKey is reliably 'metamask' for all external wallets regardless of isMetaMask flag
    if (!wallet.privateKey || wallet.privateKey === 'metamask') return;
  ...
  return (
    <AppContext.Provider value={{ wallet, profile, isDemo, darkMode, toggleTheme }}>
      <div className="app-grid">
        ...
        <Sidebar contacts={contacts} activeId={active?.id ?? null} wallet={wallet} isDemo={isDemo} profile={profile} mobileOpen={mobileSidebarOpen} onMobileClose={() => setMobileSidebarOpen(false)} onSelect={selectContact} onNew={() => { setShowNew(true); setMobileSidebarOpen(false); }} onNewGroup={() => { setShowGroup(true); setMobileSidebarOpen(false); }} onProfile={() => { setShowProfile(true); setMobileSidebarOpen(false); }} onSettings={() => { setShowSettings(true); setMobileSidebarOpen(false); }} onWallet={() => { setShowWallet(true); setMobileSidebarOpen(false); }} onLogout={handleLogout} onEditContact={setEditContact} onSearch={() => setShowSearch(true)} onLeaveGroup={handleLeaveGroup} onToggleMute={handleToggleMute} mutedGroupIds={mutedGroupIds} darkMode={darkMode} onToggleTheme={toggleTheme} chatWallpaper={chatWallpaper} onSetWallpaper={handleSetWallpaper} onChangePassword={handleChangePassword} onNewGroupCreated={(c)=>{ setContacts((p:any[])=>[c,...p]); selectContact(c); setMobileSidebarOpen(false); setManageGroupContact(c); }} onManageGroup={(c)=>setManageGroupContact(c)} />
        <main className="chat-panel">
          {/* mobile-chat-wrap: slides in from right on mobile (WhatsApp style) */}
          <div className={`mobile-chat-wrap${(active && active.address && !mobileSidebarOpen) ? ' active' : ''}`}
